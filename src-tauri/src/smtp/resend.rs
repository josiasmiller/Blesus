...
pub async fn send(
    ...
    if let Some(irt) = &outgoing.in_reply_to {
        custom_headers.insert("In-Reply-To".to_string(), json!(irt));
    }
    if let Some(refs) = &outgoing.references {
        custom_headers.insert("References".to_string(), json!(refs));
    }
    if !custom_headers.is_empty() {
        body["headers"] = serde_json::Value::Object(custom_headers);
    }
