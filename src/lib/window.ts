...
export async function restoreWindowState(): Promise<void> {
  ...
  // Show first — on Windows, setSize/setPosition silently fail on hidden windows.
  // Small delay ensures the WebView2 IPC bridge is fully ready before we call show().
  await new Promise((r) => setTimeout(r, 80));
  try {
    await win.show();
  } catch (err) {
    console.warn("restoreWindowState show failed", err);
  }
  ...
  try {
    if (w != null && h != null && w >= MIN_W && h >= MIN_H) {
      ...
      await win.setSize(new PhysicalSize(1280, 840));
    }
  ...
  try {
    // Second show call ensures the window is visible even if it was
    // re-hidden during position/size restoration.
    await win.show();
  } catch (err) {
